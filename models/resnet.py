...
class BasicBlock(nn.Module):
    ...
    groups: 这是一个用于分组卷积的参数。分组卷积可以将输入特征图的通道分成几个组，每个组独立地进行卷积操作。
    这可以减少参数数量和计算成本，同时可能有助于提升模型的性能。默认值是1，表示不使用分组卷积。
    width_per_group: 当使用分组卷积时，这个参数指定了每个组的通道数。例如，如果总通道数是128，
    groups 是4，那么 width_per_group 就是32，意味着每个组有32个通道。
           """
